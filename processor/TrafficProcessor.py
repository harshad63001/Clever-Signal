...
class TrafficProcessor:
    ...
    def cross_violation(self, frame):
        ...
        if self.light == "Green":
            cv2.rectangle(self.frame, self.zone1, self.zone2, (0, 255, 0), 2)
        else:
            cv2.rectangle(self.frame, self.zone1, self.zone2, (0, 0, 255), 2)
        ...
        for i in range(len(cropped_cars)):
            cv2.imshow('car', cropped_cars[i])
